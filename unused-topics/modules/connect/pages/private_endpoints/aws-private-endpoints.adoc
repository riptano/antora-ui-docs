= Connect to AWS Private Endpoints via {astra_db} console
...
All AWS screenshots are using legacy UI view. If you are on the new UI experience, you will have to adjust accordingly.
====
...
== Creating and referencing endpoint values between AWS and {astra_db} consoles
...
+
image::astra-db-settings-tab-private-endpoints-aws.png[{astra_db} console Settings tab with Private Endpoints section for AWS]
+
...
. {astra_db} console displays an updated Private Endpoints section.
. Click **Add Endpoint**.
. On **Add Private Endpoint**, copy the generated **Service Name**.

[TIP]
====
In {astra_db} console, keep the **Add Private Endpoint** dialog open. We'll return here with an **Endpoint ID** after creating it in AWS console.
====
...
.. Service Name: Paste in the Service Name value that you copied in {astra_db} console.
...
+
. Once accepted, AWS displays data for the added endpoint. **Copy** the generated Virtual Private Cloud (VPC) ID. You'll need to paste in that **VPC ID** value in {astra_db} console.
...
. In the Endpoint ID field, paste in the copied **VPC ID** value. Also enter a brief description of your {astra_db} / AWS endpoint.
. Click **Add Endpoint**.
+
