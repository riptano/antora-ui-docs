= Connect to Azure Private Endpoints via {astra_ui}
...
== Creating and referencing endpoint values between Microsoft Azure portal and {astra_ui}
...
+
image::astra-db-settings-tab-private-endpoints-azure2.png[{astra_ui} Settings tab with Private Endpoints section for Azure pre-configuration]
+
...
. {astra_ui} displays an updated Private Endpoints section, including a generated **Service Name**.
. Click **Add Endpoint**.
...
[TIP]
====
In {astra_ui}, keep the **Add Private Endpoint** dialog open. We'll return here with an **Endpoint ID** after creating it in Azure portal.
====
...
.. Paste into **Resource ID or alias** the **Service Name** value that you copied from {astra_ui}.
...
+
. Once validated, Azure portal displays a summary page for the added private endpoint. **Copy** the generated endpoint's **Resource ID**, which you'll later paste into the Add Private Endpoint dialog in {astra_ui}. To get the Resource ID:
