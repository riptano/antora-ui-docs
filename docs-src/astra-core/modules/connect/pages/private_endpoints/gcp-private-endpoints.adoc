= Connect to Google Cloud private endpoints via {astra_ui}
...
== Creating and referencing endpoint values between Google Cloud console and {astra_ui}

Setting up the connection between Google Cloud and {astra_db} private endpoints involves a few steps in both UIs.
...
+
image::astra-db-settings-tab-private-endpoints.png[{astra_ui} Settings tab with Private Endpoints section]
+
...
. {astra_ui} displays an updated Private Endpoints section, which includes a newly generated Service Name.
. Click **Add Endpoint**.
...
[TIP]
====
In {astra_ui}, keep the **Add Private Endpoint** dialog open. We'll return here with an **Endpoint ID** after creating it in Google Cloud Console.
...
.. Target service: Paste in the Service Name value that you copied in {astra_ui}.
...
You'll need to paste in that **PCS ID** value in {astra_ui}.
...
* For more, see link:https://cloud.google.com/vpc/docs/private-service-connect[Private Service Connect] and the https://docs.datastax.com/en/astra/docs/_attachments/devopsv2.html#operation/Private-Endpoints[DevOps API reference, window="_blank"].
