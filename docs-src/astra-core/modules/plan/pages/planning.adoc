...
include::ROOT:partial$connect_options.adoc[]
...
Use the {company} {astra_ui} to xref:manage:db/managing-db.adoc#_monitor_your_databases[get statistics and view database and health metrics].
...
If you are on the {astra_db} **Free Plan**, there is a limit of **five** databases per xref:manage:org/managing-org#_add_organizations_in_astra_db[organization]. 

Users on the {astra_db} **Pay as You Go (PAYG)** or **Enterprise** paid plan may create up to 50 databases per organization. You can have multiple organizations, such as an organization for each department in your company, to further increase the overall total number of databases.

To upgrade from a Free Plan to a paid plan, {support_url}[contact {company} Support, window="_blank"].
...
{serverless_cap} databases created using {astra_db} are automatically backed up once an hour. The hourly backups are stored for 20 days and only include sstables. This backup could exclude data written that is in the memtable and the commit log, but has not been flushed to the sstables.
...
For databases with multiple datacenters in various regions, backup management is only available for the original datacenter selected when you created the database.
//backup is managed for only the original datacenter selected when creating your database.
...
If data is accidentally deleted or corrupted, contact {support_url}[{company} Support, window="_blank"] to restore data from one of the available backups. This window ensures that the data to restore exists as a saved backup.

When restoring data, {support_url}[{company} Support, window="_blank"] allows you to restore data to the same database, replacing the current data with data from the backup. All data added to the database after the backup is no longer available in the database.
...
{astra_db} workloads for Classic databases do not have a rate limit.
...
The latest six backups are stored, providing flexibility in which point in time you can restore to, if necessary.

[IMPORTANT]
====
If the database was terminated, all data is destroyed and is unrecoverable.
====
If data is accidentally deleted or corrupted, contact {support_url}[{company} Support] within 12 hours to restore data from one of the available backups. This window ensures that the data to restore exists as a saved backup.

When restoring data, {support_url}[{company} Support] allows you to restore data to the same database, replacing the current data with data from the backup. All data added to the database after the backup is no longer available in the database.
endif::[]
