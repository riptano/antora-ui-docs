...
** AWS - supported via BYOK AWS with DevOps API (this topic) and xref:manage:org/byok-ui-aws.adoc[BYOK AWS with {astra_ui}].
** Google Cloud - supported via {astra_db} xref:manage:devops/devops-gcp-cmek.adoc[BYOK GCP with DevOps API], and xref:manage:org/byok-ui-gcp[BYOK GCP with {astra_ui}].
...
include::manage:partial$byok_aws_kms_console.adoc[]
...
Retrieves a Customer Key in an {astra_db} organization for a given cloud provider &amp; region combination. The provider value in this case should be `aws`. 
