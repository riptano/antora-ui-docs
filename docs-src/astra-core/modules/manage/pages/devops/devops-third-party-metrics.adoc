...
You can configure the export of {astra_db} metrics via the DevOps API (described in this topic), or via xref:manage:db/third-party-metrics-ui.adoc[{astra_ui}, window="_blank"].
...
+
Keep track of your `databaseId`. You'll specify it in the DevOps `POST` API call for `/v2/databases/{databaseId}/telemetry/metrics`. You can find the `databaseId` on the {astra_ui}'s dashboard.
+
Example:
+
image:database-id-dashboard.png[{astra_ui} dashboard shows database ID.]
...
* https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/GettingSetup.html[Getting set up with Amazon CloudWatch, window="_blank"]
* https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/permissions-reference-cw.html[Amazon CloudWatch permissions reference, window="_blank"]
...
The configuration payload (JSON) depends on which destination you'll use. Currently we support Prometheus `remote_write`, Kafka, and Amazon CloudWatch destinations.

To ensure that metrics are enabled for your destination app, provide the relevant properties as described below. 
...
See the {astra_ui} Dashboard for its value. You can define a variable such as `$DB_ID`, set it to your databaseId value, and then use the variable in a `curl` command.
...
With a required top-level key of `aws`, the POST payload's required properties are:
...
[TIP]
====
Reminder: In AWS, the secret key user must have the `PutMetricData` action defined as the minimum required permission. See the AWS IAM link:#putmetricdata[configuration example] presented earlier in this topic.
====
...
[source,shell, subs="attributes+"]
----
include::manage:example$devops/third-party/curl_post_cloudwatch.sh[]
----
--
...
[source,plaintext, subs="attributes+"]
----
include::manage:example$results/third-party/post_response_cloudwatch.result[]
----
--
====
...
[source,shell, subs="attributes+"]
----
include::manage:example$devops/third-party/curl_get_cloudwatch.sh[]
----
--
...
[source,plaintext, subs="attributes+"]
----
include::manage:example$results/third-party/get_response_cloudwatch.result[]
----
--
====
...
+
image:astra-health-share-option.png[{astra_ui} Health Share]
. Then select the **Export** tab:
+
image:astra-health-share-export.png[{astra_ui} Health Export]
. Click **View JSON** and then **Copy to Clipboard**:
+
image:astra-health-json-copy-to-clipboard.png[{astra_ui} Health Copy to Clipboard]
