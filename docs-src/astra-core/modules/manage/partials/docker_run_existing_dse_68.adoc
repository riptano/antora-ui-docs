=== Starting Stargate with existing DataStax Enterprse cluster
...
Use the `SIMPLE_SNITCH` option if the
  link:https://github.com/apache/cassandra/blob/cassandra-3.11/conf/cassandra.yaml#L962[endpoint_snitch, window="_blank"]
